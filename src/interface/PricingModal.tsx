...
const PricingModal: React.FC<PricingModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="modal-overlay fixed inset-0 flex items-center justify-center p-6 pointer-events-auto overflow-hidden">
      <div
        onClick={onClose}
        className="absolute inset-0 bg-black/25 backdrop-blur-xl"
      />
      <div
        className="relative w-full max-w-4xl theme-modal-panel rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] p-8 md:p-12 max-h-[90vh] overflow-y-auto"
      >
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <div className="mx-auto">
          ...
          <div className="mb-10 text-center">
            <h2 className="text-3xl font-black text-[var(--apple-text)] tracking-tight mb-2">
              Gemini API Pricing
            </h2>
                  ...
                  {reasoningModels.map(([model, pricing]) => {
                    ...
                    return (
                      <div key={model} className={`relative px-5 py-3.5 rounded-2xl border transition-all duration-300 flex items-center justify-between ${
                        isDefault ? 'bg-blue-50/80 border-blue-100 shadow-sm' : 'theme-muted border-zinc-100/60'
                      }`}>
                        {isDefault && (
                        ...
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <p className="text-xs font-bold text-[var(--apple-text)] lowercase">
                            {model}
                          </p>
                        ...
                        <div className="flex items-center gap-5 text-xs font-mono font-bold">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[var(--apple-text)]">${pricing.inputPer1M?.toFixed(2)}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-zinc-400 font-medium uppercase text-[10px] tracking-tighter">Out</span>
                            <span className="text-[var(--apple-text)]">${pricing.outputPer1M?.toFixed(2)}</span>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  <div key={type} className="space-y-6">
                    ...
                    <div className="space-y-3">
                      {typeModels.map(([model, pricing]) => {
                        ...
                        return (
                          <div key={model} className={`relative px-5 py-3.5 rounded-2xl border transition-all duration-300 flex items-center justify-between ${
                            isDefault ? 'bg-amber-50/80 border-amber-100 shadow-sm' : 'theme-muted border-zinc-100/60'
                          }`}>
                            {isDefault && (
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <span className="text-sm font-mono font-bold text-[var(--apple-text)]">
                  ...
                  <div key={type} className="space-y-6">
                    ...
                    <div className="space-y-3">
                      {typeModels.map(([model, pricing]) => {
                        ...
                        return (
                          <div key={model} className={`relative px-5 py-3.5 rounded-2xl border transition-all duration-300 flex items-center justify-between ${
                            isDefault ? `${colors.bg} ${colors.borderLight} shadow-sm` : 'theme-muted border-zinc-100/60'
                          }`}>
                            {isDefault && (
