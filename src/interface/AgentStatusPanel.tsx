...
const AgentStatusPanel: React.FC<AgentStatusPanelProps> = ({ agentIndex }) => {
  ...
  const { isThinking, isChatting, selectedNpcIndex } = useUiStore();
  ...
  const isRespondingLive = isThinking && isChatting && selectedNpcIndex === agentIndex;
    ...
    <div className="flex flex-col h-full p-6">
      ...
      {isRespondingLive ? (
        <div className="mb-6">
          ...
          <p className="text-sm leading-snug font-medium" style={{ color: 'var(--apple-text-secondary)' }}>
            Streaming a reply in chat…
          </p>
        </div>
      ) : activeTask ? (
        <div className="mb-6">
        ...
        <div className="mb-6">
          ...
          {tasks.filter((t) => t.assignedAgentId === agentIndex && t.status === 'scheduled').length > 0 && (
            <p className="text-[11px] mt-2" style={{ color: 'var(--apple-text-secondary)' }}>
              {tasks.filter((t) => t.assignedAgentId === agentIndex && t.status === 'scheduled').length} scheduled
            </p>
          )}
        </div>
      )}
