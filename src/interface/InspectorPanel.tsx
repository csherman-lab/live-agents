...
const InspectorPanel: React.FC<InspectorPanelProps> = ({ isFloating }) => {
  ...
  return (
    <div className={`${isFloating ? 'w-full h-full max-h-[85vh] self-end rounded-2xl shadow-2xl border border-white/30' : 'w-[min(320px,28vw)] h-full border-l border-black/5'} panel-sidebar flex flex-col pointer-events-auto shrink-0 relative z-30 overflow-hidden transition-apple`}>
      {!agent ? (
        !isFloating && <ProjectView />
      ) : (
        <>
          {/* Header Section */}
          <div className="px-4 py-3.5 border-b app-header">
            <div className="flex flex-col gap-4">
              {/* Agent Title Row */}
              <div className="flex items-center gap-4">
                <div className="shrink-0 rounded-2xl p-0.5 theme-muted border" style={{ borderColor: 'var(--apple-border)' }}>
                  <Avatar
                ...
                <div className="flex flex-col min-w-0">
                  <h2 className="text-lg font-semibold text-[var(--apple-text)] leading-tight truncate">
                    {agent.name}
                  </h2>
              ...
              {needsInput && isChatting && (
                ...
                >
                  ...
                  <p className="text-[12px] font-bold text-[var(--apple-text)] leading-tight mt-1.5">
                    {isLeadAgentIdle
              ...
              {needsInput && !isChatting ? (
                <div className="flex flex-col gap-3 p-4 theme-chip rounded-xl animate-in fade-in slide-in-from-top-1 shadow-sm">
                  ...
                  <div className="flex flex-col gap-2">
                    <p className="text-[12px] font-bold text-[var(--apple-text)] leading-tight">
                      {isLeadAgentIdle
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 bg-appleBlue hover:opacity-90 active:scale-95 disabled:opacity-50 text-white px-4 py-3 rounded-xl text-[12px] font-semibold transition-apple shadow-sm mt-1"
                    >
                      {isLeadAgentIdle ? (
                        <>
                  ...
                  {agent.index === system.user.index ? (
                    ...
                    <button
                      ...
                      className={`w-full h-10 px-4 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 text-[10px] font-black uppercase tracking-widest ${canChat
                          ? 'bg-appleBlue text-white border-none shadow-md'
          ...
          <div className="flex-1 overflow-y-auto relative min-h-0 theme-muted">
            {isChatting ? (
              <div className="flex flex-col h-full theme-panel">
                <div className="flex-1 overflow-y-auto">
                  <ChatPanel />
                </div>
                <div className="p-3 border-t theme-panel shrink-0" style={{ borderColor: 'var(--apple-border)' }}>
                  <button
                    onClick={handleEndChat}
                    className="w-full h-10 px-4 bg-appleBlue hover:opacity-90 text-white rounded-xl flex items-center justify-center gap-2 transition-apple active:scale-95 text-[12px] font-semibold shadow-sm"
                  >
