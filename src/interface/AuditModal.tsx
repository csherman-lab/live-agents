...
export const AuditModal: React.FC<AuditModalProps> = ({ taskId, isOpen, onClose, viewOnly }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 md:p-12">
      {/* Backdrop */}
      <div
        className="modal-overlay absolute inset-0 bg-black/25 backdrop-blur-xl animate-in fade-in duration-500"
        onClick={onClose}
      />

      <div className="relative theme-modal-panel w-full max-w-4xl max-h-full rounded-[32px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] flex flex-col overflow-hidden animate-in zoom-in-95 fade-in duration-500 ease-out fill-mode-both">

        <div className="px-8 py-8 flex items-center justify-between theme-panel shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-sm font-semibold text-[var(--apple-text)]">{agent?.name}</span>
                {!isViewMode && (
              ...
              <h2 className="text-xl font-semibold text-[var(--apple-text)] tracking-tight leading-tight">
                {task.title}
              </h2>
              {isViewMode && (
          ...
          >
            <X size={20} className="text-zinc-400 group-hover:text-[var(--apple-text)] transition-colors" />
          </button>
        </div>
              ...
              <section className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-1 h-4 bg-appleBlue rounded-full" />
                    <h3 className="text-[11px] font-semibold text-[var(--apple-text-secondary)] uppercase tracking-wider">
                ...
                <div className="p-8 theme-muted rounded-3xl border min-h-[350px] shadow-inner" style={{ borderColor: 'var(--apple-border)' }}>
                  <div className="markdown-content text-[var(--apple-text)] text-sm leading-relaxed">
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${selectedRevisionIndex === idx
                          ? 'bg-appleBlue border-appleBlue text-white shadow-md'
                          : 'bg-white border-zinc-100 text-[var(--apple-text)] hover:border-zinc-300 hover:bg-zinc-50'}
                      `}
                    >
        ...
        {!isViewMode && selectedRevisionIndex === null && (
          <div className="px-8 py-4 border-t theme-panel" style={{ borderColor: 'var(--apple-border)' }}>
        ...
        {/* Footer Actions */}
        <div className="px-8 py-8 theme-muted border-t flex items-center justify-end gap-4 shrink-0" style={{ borderColor: 'var(--apple-border)' }}>
          {(isViewMode || selectedRevisionIndex !== null) ? (
            <button
              onClick={selectedRevisionIndex !== null ? () => setSelectedRevisionIndex(null) : onClose}
              className="h-12 px-10 bg-appleBlue text-white rounded-2xl text-[13px] font-semibold transition-apple hover:opacity-90 active:scale-95 flex items-center gap-2 shadow-sm"
            >
