...
function TaskCard({ task }: { task: Task; key?: string }) {
  ...
  return (
    <div key={task.id} className="apple-card p-3 space-y-2 group relative hover:shadow-md transition-apple">
      <div
        className="flex items-start justify-between gap-1 cursor-pointer"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <h3 className="text-xs text-[var(--apple-text)] leading-snug font-semibold flex-1">
      ...
      {isExpanded && (
        <p className="text-[11px] text-[var(--apple-text-secondary)] leading-relaxed theme-muted p-2 rounded border animate-in fade-in slide-in-from-top-1 duration-200" style={{ borderColor: 'var(--apple-border)' }}>
          {task.description}
        </p>
      )}
      ...
      <div className="flex items-center justify-between gap-x-2 gap-y-1 pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {task.status === 'in_progress' && (
            <span
              className="inline-block text-[10px] font-semibold rounded-full px-2 py-0.5 shadow-sm border whitespace-nowrap"
              style={{
                color: USER_COLOR,
...
export function KanbanPanel({ height = 320 }: KanbanPanelProps) {
  ...
  return (
    <div
      className="w-full panel-sidebar border-t flex flex-col pointer-events-auto shrink-0 relative"
      style={{ height, borderColor: 'var(--apple-border)' }}
    >
      {tasks.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center px-8 py-6 text-center">
          ...
          <p className="text-[12px] mt-1.5 max-w-[280px] leading-relaxed" style={{ color: 'var(--apple-text-secondary)' }}>
            Once your lead agent reviews the brief, tasks will show up here as the team works.
          </p>
        </div>
      ) : (
      <div className="flex-1 overflow-x-auto overflow-y-hidden theme-muted">
        <div className="flex h-full min-w-max px-5 py-4 gap-4">
          {COLUMNS.map(({ status, label }) => {
            const colTasks = tasks.filter((t) => t.status === status)
            return (
              <div key={status} className={`w-52 flex flex-col gap-3 ${status === 'on_hold' && colTasks.length > 0 ? 'ring-2 ring-orange-300/60 rounded-xl p-1 -m-1 bg-orange-50/30' : ''}`}>
                <div className="flex items-center justify-between shrink-0 select-none">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400 leading-none">
                      {label}
                    </span>
                    <span className="px-1.5 py-0.5 theme-chip text-[var(--apple-text-secondary)] text-[9px] font-bold rounded-md min-w-4.5 text-center">
                      {colTasks.length}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col gap-2 overflow-y-auto pr-1">
                  ...
                  {colTasks.length === 0 && (
                    <div className="border border-dashed rounded-lg p-4 flex items-center justify-center select-none" style={{ borderColor: 'var(--apple-border)' }}>
                      <span className="text-[10px] font-bold text-[var(--apple-text-secondary)] uppercase tracking-widest">Empty</span>
                    </div>
                  )}
                </div>
              </div>
