...
export function FinalOutputModal() {
  ...
  const renderContent = () => {
    ...
    if (finalAssetType === 'image' && finalAssetContent) {
      return (
        <div className="space-y-4">
          ...
          <div className="p-4 theme-muted rounded-xl border" style={{ borderColor: 'var(--apple-border)' }}>
            <p className="text-[10px] font-black uppercase tracking-wider text-[var(--apple-text-secondary)] mb-1">PROMPT USED:</p>
            <p className="text-xs text-[var(--apple-text-secondary)] italic leading-relaxed">{finalOutput || "No prompt metadata available."}</p>
          </div>
        </div>
      );
    }
    ...
    if (finalAssetType === 'audio' && finalAssetContent) {
      return (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row items-center gap-3 p-3 theme-panel rounded-2xl shadow-sm">
  ...
  return (
    <div
      className="modal-overlay fixed inset-0 flex items-center justify-center bg-black/25 backdrop-blur-xl p-4"
      onClick={() => setFinalOutputOpen(false)}
    >
      <div
        className="theme-modal-panel border rounded-[32px] w-180 max-w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden"
      ...
      >
        <div className="flex items-center justify-between px-8 py-6 border-b theme-panel" style={{ borderColor: 'var(--apple-border)' }}>
          ...
          <button
            onClick={() => setFinalOutputOpen(false)}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-zinc-100 text-zinc-400 hover:text-zinc-700 transition-colors transition-all"
            aria-label="Close"
          >
            <X size={18} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-8">
          <div className="markdown-content text-sm text-[var(--apple-text)] leading-relaxed font-sans">
            {renderContent()}
          </div>
        </div>

        {/* Footer */}
        <div className="px-8 py-6 border-t theme-panel flex flex-col gap-6" style={{ borderColor: 'var(--apple-border)' }}>
          ...
          <div className="flex justify-between items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-4 py-3 bg-black/[0.05] text-zinc-700 rounded-2xl text-[10px] font-bold uppercase tracking-wider hover:bg-black/[0.08] active:scale-[0.98] transition-all"
              >
                <Package size={14} />
                Export All
              </button>
