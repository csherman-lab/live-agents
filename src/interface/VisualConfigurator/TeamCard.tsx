...
import { DEFAULT_MODELS, getAvailableModels, getDefaultModels, getProviderConfig, getSupportedOutputTypes, normalizeProviderId } from '../../core/llm/constants';
...
export const TeamCard: React.FC<TeamCardProps> = ({
  ...
  const { updateSystem, deleteCustomSystem, selectedAgentSetId } = useTeamStore();
  const { llmConfig } = useUiStore();
  const providerId = normalizeProviderId(llmConfig.provider);
  const supportedOutputTypes = getSupportedOutputTypes(providerId);
  ...
  useEffect(() => {
    if (isEditing) {
      setLocalEditData({
        ...
        outputType: supportedOutputTypes.includes(system.outputType || 'text')
          ? (system.outputType || 'text')
          : 'text',
        outputModel: system.outputModel || getDefaultModels(providerId).text,
  ...
  const confirmDelete = (e: React.MouseEvent) => {
    ...
    if (system.id === selectedAgentSetId) {
      scene?.resetScene();
      requestTeamSwitch(DEFAULT_AGENTIC_SET_ID);
    }
  ...
  return (
    <div
      ...
      className={`group relative p-3.5 rounded-2xl transition-all cursor-pointer border-[3px] ${isSelected ? 'theme-muted shadow-sm' : 'theme-panel hover:border-zinc-200/50'
        }`}
      style={{
      ...
      <div className="flex flex-col">
        ...
        <div className="flex flex-col flex-1 min-w-0">
            ...
            <div className="space-y-2 mb-3" onClick={(e) => e.stopPropagation()}>
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full theme-input text-[13px] font-medium rounded-xl px-2.5 py-1.5 outline-none transition-colors"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full theme-input text-[13px] font-medium rounded-xl p-2.5 outline-none resize-none h-20 leading-snug transition-colors"
                  onFocus={(e) => e.target.style.borderColor = USER_COLOR}
                  onBlur={(e) => e.target.style.borderColor = '#f4f4f5'}
                />
              </div>
              ...
              {supportedOutputTypes.length === 1 && (
                <p className="text-[10px] text-zinc-500 leading-relaxed px-1">
                  {getProviderConfig(providerId).label} supports text deliverables only. Use Google Gemini in Settings for image, music, or video teams.
                </p>
              )}
          ...
          <div className={`flex items-center justify-between mt-auto pt-2 ${isEditing ? 'border-t border-zinc-100/30' : ''}`}>
            <div className="flex items-center gap-1.5 px-2 py-1 theme-chip text-[8px] font-black text-[var(--apple-text-secondary)] rounded-lg">
