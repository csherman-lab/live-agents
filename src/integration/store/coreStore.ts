...
interface CoreState {
  ...
  viewMode: 'overview' | 'simulation' | 'design';
  isLogOpen: boolean
  ...
  resetProject: () => void;
  setViewMode: (mode: 'overview' | 'simulation' | 'design') => void;
...
export const useCoreStore = create<CoreState>()(
  persist(
    (set) => ({
      ...
      setIsGeneratingAsset: (isGenerating) =>
        set((s) => ({
          isGeneratingAsset: isGenerating,
          generationError: isGenerating ? null : s.generationError,
        })),
      setGenerationError: (error) => set({ generationError: error, isGeneratingAsset: false }),
      ...
      addResponseLog: (entry) =>
        set((s) => {
          ...
          let nextCostEvents = s.costEvents;
          let callCost = 0;
          ...
          if (entry.usage) {
            ...
            callCost = calculateCost(entry.usage.promptTokens, entry.usage.completionTokens, modelName, durationOrCount);
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        phase: state.phase,
        tasks: state.tasks,
        actionLog: state.actionLog.slice(-40),
...
useTeamStore.subscribe((state, prevState) => {
  if (state.selectedAgentSetId !== prevState.selectedAgentSetId) {
    if (useUiStore.getState().skipProjectResetOnTeamChange) return;
