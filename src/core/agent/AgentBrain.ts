...
import { createMediaProvider, createTextProvider } from '../llm/providers/createProvider';
import { getProviderConfig, providerSupportsOutput } from '../llm/constants';
...
import { friendlyApiError, isApiKeyError } from '../../utils/apiErrors';
...
export class AgentBrain {
  ...
  public async think(prompt: string, options: ThinkOptions = {}): Promise<{ text: string, toolCalls: any[] }> {
    ...
    let streamMessageIndex: number | null = null;
    ...
    try {
      ...
      if (useUiStore.getState().isDemoMode) {
        throw new Error('Demo mode — add an API key in Settings to run agents');
      }
      if (!llmConfig.apiKey) {
        throw new Error(`${getProviderConfig(llmConfig.provider).keyLabel} is required`);
      }
      const provider = createTextProvider(llmConfig);
      ...
      let response: Awaited<ReturnType<typeof provider.generateCompletion>>;

      if (options.isChat && 'generateCompletionStream' in provider && provider.generateCompletionStream) {
        useUiStore.getState().setAgentStatus(this.host.data.index, 'talking');
        const streamMsg: LLMMessage = {
          role: 'assistant',
          content: '',
          metadata: { streaming: true }
        };
        this.history.push(streamMsg);
        streamMessageIndex = this.history.length - 1;
        this.syncToStore();

        response = await provider.generateCompletionStream(
          messages,
          toolDefs,
          systemPrompt,
          model,
          {
            onTextDelta: (text) => {
              ...
              this.history[streamMessageIndex] = {
                ...this.history[streamMessageIndex],
                content: text,
                metadata: { streaming: true }
              };
      ...
      if (streamMessageIndex !== null) {
        this.history[streamMessageIndex] = assistantMessage;
      } else {
      ...
      if (streamMessageIndex !== null) {
        const partial = this.history[streamMessageIndex];
        if (partial?.metadata?.streaming && !partial.content) {
          this.history.splice(streamMessageIndex, 1);
        } else if (partial?.metadata?.streaming) {
          this.history[streamMessageIndex] = {
            ...partial,
            metadata: undefined,
          };
      ...
      if (isApiKeyError(errMsg)) useUiStore.getState().openApiSettings(errMsg);
      throw error;
    } finally {
      this.isThinking = false;
      if (options.isChat) {
        useUiStore.getState().setAgentStatus(this.host.data.index, 'idle');
      }
  ...
  public async processFinalAsset(prompt: string, options: any) {
    ...
    try {
      ...
      if (activeTeam.outputType !== 'text' && !providerSupportsOutput(llmConfig.provider, activeTeam.outputType)) {
        throw new Error(
          `${getProviderConfig(llmConfig.provider).label} only supports text deliverables. ` +
          'Switch your team output to Text in Teams, or use Google Gemini in Settings for image/music/video.',
        );
      }

      const mediaProvider = createMediaProvider(llmConfig) as any;
      ...
      if (activeTeam.outputType === 'image') {
        const result = await mediaProvider.generateImage(prompt, model, (msg: string) => {
          console.log(`[System:Image] ${msg}`);
        }, options, core.referenceImages);
        assetContent = result.data || '';
        usage = result.usage;
      } else if (activeTeam.outputType === 'music') {
        const result = await mediaProvider.generateAudio(prompt, model, (msg: string) => {
          console.log(`[System:Audio] ${msg}`);
        });
        assetContent = result.data || '';
        usage = result.usage;
      } else if (activeTeam.outputType === 'video') {
        const result = await mediaProvider.generateVideo(prompt, model, (msg: string) => {
      ...
      const errMsg = error instanceof Error ? error.message : String(error);
      core.setGenerationError(
        /api.?key|invalid|401|403/i.test(errMsg)
          ? 'API key error — check Settings'
          : /quota|rate|429/i.test(errMsg)
